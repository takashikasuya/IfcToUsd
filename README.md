...
IFC（建築BIMデータ）を構造化された [OpenUSD](https://openusd.org/) ステージへ変換し、
その結果をボクセル表現・glTF・自己完結型 Web ビューワーとして閲覧できるようにするプログラム。
...
1. **IFC→USD変換**（`convert`）: IFCの空間階層・ジオメトリ・マテリアル・メタデータを、
   忠実にUSDへ変換する。これが正本（single source of truth）になる。
2. **変換結果の統合閲覧**: 変換したUSDを、ボクセル化・volume場（SDF）解析・軽量な配信形式へと
   派生させ、複数の閲覧手段を提供する。
...
- `convert`が生成するUSDが常に唯一の正本。`voxelize`/`export-gltf`/`serve`はいずれもこの正本を
  読むだけで、書き換えない（PointInstancerレイヤーも既存USDへの参照を持つ独立ファイル）。
- `serve`は`export-gltf`相当のGLBと`scene.json`（階層・customDataを平坦化したもの）、
  ボクセル化可能な要素があれば`voxels.json`を組み立てて配信する。Webビューワーは
  USDを直接パースせず、これらの軽量な派生ファイルだけを読む。
- 占有ボクセルグリッドから narrow-band SDF（符号付き距離場）を構築し、選択した要素の
  水平断面をWebビューワー上へオーバーレイ表示する機能（`serve --sdf-slices`、後述）もある。
...
# 要素ごとのSDF水平スライスも生成する（既定off、追加の計算コストがかかるためopt-in）
uv run ifc2usd serve output/ToyodaLab_structured.usda --sdf-slices
```

機能:

- 階層ツリー表示・要素ごとの表示/非表示切替
- 3Dクリックまたはツリーからの選択（GUID/class/customDataを表示するプロパティパネル付き。
  ボクセル専用表示モードでも、選択した要素のボクセルがハイライトされる）
- メッシュ/ボクセル/両方の表示モード切替とボクセルLOD切替
  （ボクセル化可能な要素があれば自動生成される）
- 断面クリップ（高さスライダーで、その高さより上を隠す）
- `--sdf-slices`指定時: 選択した要素のnarrow-band SDF（符号付き距離場）水平断面を
  半透明オーバーレイ表示（白=表面、青=内部、橙=外部）するトグル+高さスライダー
...
- `ifc2usd/` — IFC→USD 変換・ボクセル化・glTF/Webビューワー配信の Python パッケージ / CLI（本体）
  ...
  - `sdf.py` — 占有ボクセルグリッドからnarrow-band SDF（符号付き距離場）を構築
  - `sdf_slice.py` — 要素ごとのSDFを水平断面グリッドへ変換（Webビューワーのオーバーレイ用）
  - `gltf.py` — USD→glTF(GLB) エクスポート
  - `scene_index.py` — USD→`scene.json`（Web ビューワー用の階層・customData抽出）
  - `serve.py` — Web ビューワー用の静的ファイル一式（GLB/scene.json/voxels.json、
    `--sdf-slices`指定時はSDF断面JSONも）の組み立てとローカル HTTP サーバー
  - `viewer/` — three.js ベースの Web ビューワー（ビルド不要、静的ファイル）。
    GLB表示・階層ツリー・クリック選択（ボクセル個別ハイライト含む）・ボクセル描画・
    表示モード/LOD切替・断面クリップ・SDFスライスオーバーレイを実装
