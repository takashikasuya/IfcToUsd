...
ノートブック形式（v1、暗黙）の後継。v1 からの変更点: `version`/`upAxis`/`units` の明示、
      ...
      "elements": [
        {
          ...
          "indices": [1234, 1237, ...]         // morton3(ix, iy, iz)。ix = floor((x-origin.x)/size)
        }
      ]
    }
  ]
...
- `indices` は Morton/Z-order（3軸 21bit まで = 一辺 2,097,152 ボクセル）。
  ソート済みで格納する（範囲クエリ・差分・圧縮に有利）。
- 属性の詳細（プロパティセット）は JSON に**重複格納しない**。GUID で USD / scene.json
  側を参照する（正本の一元化）。
- 後方互換: v1（`voxelSize`/`offset`/Morton color）はビューワーで読み込み時に v2 へ
  変換するローダーを用意する（`GLTF_to_Voxel.ipynb` の既存出力を捨てない）。
